...
These inevitable problems make it difficult for your company to conduct analyses. To prevent these issues, you’ll need a data engineer familiar with the data warehouse, and how users are querying the source. This article will go in dept on these issues and how to address them with routine maintenance.
...
We may add a new field to track our customer information in Salesforce that are inline with our new company objectives, say, tracking account activities through the services we provide. From here, we can see what services are most popular with our customers, then, offer special promotions on these services during seasonal trends where we see a fall in purchases in order to increase sales.

### Why do new metrics cause issues in a data warehouse?

When engineers or analysts create new tables, columns, or views to track metrics they do not always follow the naming convention set out for the Data Warehouse. This makes it difficult to be interpreted by an analyst unfamiliar with the new metrics.

This can also create duplicate work, say you created a view for your support team but the view along with the pertinent information inside the view do not distinguish exactly what this view is for. Users looking to query this view may not know the view exists, so they may recreate this view.
...
Backfill data whenever we can determine what the values should be.

In a dimension table we might be able to determine the value based off of other columns or there is an obvious default value to plug in. If we are not able to determine or have no obvious default value leave the value as null. Do consider the impact of nulls, such as in [aggregation](https://dataschool.com/how-to-teach-people-sql/how-sql-aggregations-work/)). If this will impact your query try determining a stand in value to indicate you could not backfill it.

In a measures table the same principles hold if it can be determined backfill it. Since measures are more often aggregated the impact of nulls can be even greater. The other negative of nulls in a measures table that has time based data is that it limits your analysis to when you added the new column. Sometimes this issue can be overcome by bringing in data from previous data sources or by imputing values based on overall statistics or dimensions of each row.

#### Naming Conventions

We also need to ensure these views, tables and columns follow the data warehouse’s naming conventions. At Chartio, we follow a naming convention when adding new metrics or updating metrics and making sure the name captures the updated metrics purpose: ([Link to Naming Convention and Style Guide](https://dataschool.com/data-governance/single-source-of-truth/)). We recommend publishing your style guide and distributing it among your employees as familiarity with the process keeps the naming convention intact.
...
Not being granted access to the data you need completely stops someones analysis. Not removing someone’s access can be a legal liability. Having the right permissions is a line that should be carefully considered, but not too much caution that it becomes a bottleneck.

Consider the scenario where you have users working on multiple BigQuery projects and you’re worried about query costs. In order to avoid users abusing queries and bringing up the cost, you’ll want to create a custom quota. As the BigQuery documentation outlines, a [custom quota](https://cloud.google.com/bigquery/docs/custom-quotas) will manage costs by specifying a limit on the amount of query data processed per day. This can be set at the project-level, or, at the user-level. This type of permissioning is aimed at price reduction, but we can also have permissionings aimed at keeping data intact, or, for privacy concerns.
...
Permission issues happen when BI Tool or Data Warehouse access does not mirror employee status. This happens when:
...
Making sure your BI Tools access levels mirror the employment status of your employees helps you track users and prevent security issues. The main priority of permissioning is to prevent users from being able to access sensitive information, or, from accidentally deleting data that can’t be recovered.
...
We can deploy the [EXPLAIN command and the EXPLAIN ANALYZE command](https://dataschool.com/sql-optimization/optimization-using-explain/). The _EXPLAIN_ command shows the generated query plan but does not run the query. To see the results of actually executing the query, you can use the _EXPLAIN ANALYZE_ command. Based on the output, you can decide to create an index to speed up the query time. To learn more about indexing and the various types, read more on this article on [indexing](https://dataschool.com/sql-optimization/how-indexing-works/).

#### Identify common queries and create views

In Postgres we can use pg_stat_statements to group identical queries and find optimization opportunities. Pg_stat_statements stores queries that are run against your Postgres instance, and saves the query, the execution time, the underlying reads/writes along with the variables stripped out of the query. This will allow us to determine what type of data our users want and we can optimize highly used queries.

Turn this common query into a view so that the query is run on a schedule and then users can query the [view](https://dataschool.com/sql-optimization/views/). A view can significantly cutdown the need to write a complex query if the view captures the necessary information. This can help with a bump in performance as well if you decide to add an index to the view. Finally, a view can help users unfamiliar with the structure of the data warehouse if what they need to query has been consolidated to a single place to query. You can grant specific users access to this view say, if they are from a specific department and this view reflects all the necessary information they need to query. Once creating the view, you can give access to those users to query the view.
...
* As you vet your metrics and find some need to be updated/pre-aggregated, make sure they’re named properly.
