...
From the previous chapters we established what decisions this dashboard will support. What metrics will inform those decisions and which visualizations best showcase those metrics. However the biggest challenge in dashboard design can be the “How”, as in how do we get the data to build each of the visualizations.
...
Most data is messy data. It may have obvious issues such as missing values that need to be dealt with. It may have more mysterious issues, such as manually updated fields for certain customers. Whatever the cause, all these issues need to be addressed or documented before being used in a visualization.
...
While the previous method showed us obviously wrong values there can be more subtle [outliers](https://dataschool.com/fundamentals-of-analysis/what-is-an-outlier/) and potentially wrong values that you may want to address.
...
[![shows quartile split and the interquartile range](/assets/images/how-to-design-a-dashboard/finding_the_data_that_builds_metrics/IQR.png)](http://sphweb.bumc.bu.edu/otlt/mph-modules/bs/bs704_summarizingdata/bs704_summarizingdata7.html)
...
* Slight errors will effect a query’s grouping or filtering: ‘Hamburger’, ‘Hamburdr’, ‘Hanburger’

#### Various capitalization

* Any difference could effect a query’s grouping or filtering: ‘Hamburger’, ‘HamBurger’, ‘hamburger’’
...
Sometimes you do not have the data you need for the metric in your database. This can be a huge roadblock but there are a few ways forward. First we need to know:
...
If historical data is of no concern consider working with the Data Gatekeeper to start to tracking the data for the metrics you want. This might involve engineering to implement the new tracking which will delay the project from completion based on the engineering resources available to help.
...
If we want historical data but we do not have the data for the metric we wanted we can use proxy metrics. Proxy Metrics are metrics that give us the same or similar information to what we wanted when we can’t measure the desired metric directly.
...
While Proxy Metrics are not exact they can give us a good estimation. Beware though when using proxy metrics since they may be measuring things you do not expect and can lead you to make bad decisions.
